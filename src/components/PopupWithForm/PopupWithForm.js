...
export default function PopupWithForm({
  ...
  const fadeIn = "popup_transition_fadein";
  ...
  useEffect(() => {
    const overlayClose = (e) => {
      if (e.target.classList.contains("popup_transition_fadein")) {
        // onClose();
        setIsSignInPopup(false);
        setIsSignUpPopup(false);
      }
  ...
  return (
    <section className={`popup ${fadeIn}`} id={`${name}-popup`}>
      <div className={`popup__container`}>
        ...
        <div className={`popup__form-container`}>
          ...
          <p className="popup__link">
            {" "}
            or{" "}
            <Link to="/" className="popup__link-span" onClick={onSpanClick}>
              {linkName}
            </Link>
          </p>
        </div>
